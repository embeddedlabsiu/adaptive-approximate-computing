...
int initializeParameters(unsigned int loopID)
{
 ...
 // printf("\n******SIZE OF UNISGNED INT %d*****\n",sizeof(unsigned int));
  ...
  int II = kernelsize/(16); // Edited by python script
  int epiLength = episize/16; // Edited by python script
  int prolength = prosize/16; // Edited by python script
  *(initCGRA + 7*(loopID-1) + 2) = II;
  *(initCGRA + 7*(loopID-1) + 3) = epiLength;
  *(initCGRA + 7*(loopID-1) + 4) = prolength;
  *(initCGRA + 7*(loopID-1) + 6) = livevar_st_size/16; // Edited by python script
