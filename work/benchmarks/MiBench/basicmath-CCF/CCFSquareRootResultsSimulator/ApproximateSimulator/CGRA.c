	...
	while ( newLineCounter < 3 )
	{
		...
		if ( fileBuffer[ tempLoopCounter ] == ',' || fileBuffer[ tempLoopCounter ] == '\n' )
		{
			...
			case 0:
				...
				description->adder.approximatedNumberOfBits = 4;
				description->adder.approximationType = AdderTypesLpCLibMethod;
				description->adder.lpcApproximationMethod = LPCLibraryMethodFirstApproximation;
				break;
			...
			case 2:
				...
				description->subtractor.approximationType = SubtractorTypesStandardSubtractor;
...
void createPEDescriptionFile( const char* fileToOpen, const CGRA cgra, size_t cgraSize )
{
	...
	fprintf( file, "Total Number of CGRA_PE:36\n" );
	fprintf( file, "%s", defaultValue );
