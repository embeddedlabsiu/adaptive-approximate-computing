...
// parse the user's arguements
	while ( ( opt = getopt_long( argc, argv, "n:t:", longOptions, NULL ) ) != -1 )
	{
		switch ( opt )
		{
		case NumberOfBits:
    ...
    int gray_size = sobelFilter(rgb, &gray, &sobel_h_res, &sobel_v_res, &contour_img, width, height, numberOfBits, tolerance );
