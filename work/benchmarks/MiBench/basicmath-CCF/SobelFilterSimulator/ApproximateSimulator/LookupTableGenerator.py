...
if __name__ == '__main__':
    ...
    csvData = pd.read_csv( arguments.file, names=[ 'N', 'X', 'Actual', 'Zero_Error', 'First_Error', 'Third_Error' ] )
    counter = 0
    ...
    for row in csvData.values:
        ...
        if ( counter == 1 ):
            continue
        zero, first, third = float( row[ 3 ] ), float( row[ 4 ] ), float( row[ 5 ] )
    ...
    arrayDeclaration = 'int lookupTable' + csvData.values[ 1 ][ 0 ] + '[' + str( arraySize ) + ']''[2] = { '
